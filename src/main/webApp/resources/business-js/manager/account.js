				...
				success:function(data){
					...
					if(data!=null){
						...
						for(var i=0;i<data.rows.length;i++){
							var bt = "<button class='bt' onclick=editAccount('"+list[i].desEntryId+"') >编辑</button>" +
									"<button class='bt' onclick=deleteAccount('"+list[i].desEntryId+"') >删除</button>";
							var status = list[i].status;
							if(status=='ENABLE'){
								status = "启用";
								bt = bt + "<button class='bt' onclick=changeAccount('"+list[i].desEntryId+"','DISABLE') >禁用</button>";
							}
							else{
								status = "禁用";
								bt = bt + "<button class='bt' onclick=changeAccount('"+list[i].desEntryId+"','ENABLE') >启用</button>";
							}
							var add = "<tr>"+
